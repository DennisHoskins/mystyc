...
} from 'mystyc-common/interfaces/user-astrology-data.interface';
import { 
  Sign, 
  PlanetType, 
  ZodiacSignType,
...
export class UserAstrologyService {
  ...
  async getUserAstrologyData(firebaseUid: string): Promise<UserAstrologyData | null> {
    logger.info('Building complete user astrology data', { firebaseUid }, 'UserAstrologyService');
    ...
    try {
      ...
      logger.info('User astrology data compiled successfully', { 
        firebaseUid,
      ...
      logger.error('Failed to build user astrology data', {
        firebaseUid,
        error
      }, 'UserAstrologyService');
