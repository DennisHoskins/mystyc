...
export class AdminDailyContentStatsService {
  ...
  async getGenerationStats(query?: DailyContentStatsQueryDto): Promise<DailyContentGenerationStats> {
    logger.info('Generating daily content generation stats', { query }, 'AdminDailyContentStatsService');
    ...
    try {
      ...
      const [statusResult, timeResults, failureResults] = await Promise.all([
        this.dailyContentModel.aggregate(statusPipeline),
        this.dailyContentModel.aggregate(timePipeline),
        this.dailyContentModel.aggregate(failurePipeline)
      ]);
      ...
      logger.error('Failed to generate daily content generation stats', {
        error: error.message,
        query
      }, 'AdminDailyContentStatsService');
      throw error;
  ...
  async getSourceStats(query?: DailyContentStatsQueryDto): Promise<DailyContentSourceStats> {
    logger.info('Generating daily content source stats', { query }, 'AdminDailyContentStatsService');
    ...
    try {
      ...
      logger.error('Failed to generate daily content source stats', {
        error: error.message,
        query
      }, 'AdminDailyContentStatsService');
      throw error;
  ...
  async getTimelineStats(query?: DailyContentStatsQueryDto): Promise<DailyContentTimelineStats> {
    logger.info('Generating daily content timeline stats', { query }, 'AdminDailyContentStatsService');
    ...
    try {
      ...
      logger.error('Failed to generate daily content timeline stats', {
        error: error.message,
        query
      }, 'AdminDailyContentStatsService');
      throw error;
  ...
  private buildDateFilter(query?: DailyContentStatsQueryDto): any {
