...
import { BillingPortalRequest } from '@/interfaces/user-requests.interface';
...
export async function POST(
  ...
  try {
    logger.log(`[mystyc] Customer billing portal attempt started`);

    const body: BillingPortalRequest = await request.json();
    ...
    if (!session) {
      logger.log('[mystyc] Customer billing portal: No Current Session');
    ...
    if (!nestResponse.ok) {
      logger.error('[mystyc] Failed to post subscription cancellation from Nest:', nestResponse.status);
      throw new InvalidSessionError(`[mystyc] Failed to post subscription cancellation from Nest: ${nestResponse.status}`);
    }
