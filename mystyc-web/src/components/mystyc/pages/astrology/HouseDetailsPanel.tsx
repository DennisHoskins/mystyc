...
export default function HouseDetailsPanel({ house, className } : { house: HouseComplete | null | undefined, className?: string }) {
  ...
  const allWordsLower = [...lifeAreaWords, ...keywordWords].map(w => w.toLowerCase());
  ...
  return (
    <Panel className={`${className} space-y-2`}>
      <div className='flex items-center space-x-2'>
        ...
        <Heading level={3}>{getRomanNumeral(house.houseNumber)}: {house.name}</Heading>
      </div>
      <Text variant='small'>
        {uniqueWords}
      </Text>
      <Text variant='muted'>{house.description}</Text>
    </Panel>
  );
}