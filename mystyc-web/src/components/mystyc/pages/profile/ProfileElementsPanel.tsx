...
export default function ProfileElementsPanel({ elementData } : { elementData: ElementComplete[] }) {
  const uniqueElementData = Array.from(new Map(elementData.map(element => [element?.element, element])).values());

  return (
    <Panel className="flex flex-col space-y-10">
      {uniqueElementData.map((element) => (
        <div key={element?.element} className="flex flex-col space-y-2">
