...
export class UserRolesService {
  ...
  async promoteToAdmin(firebaseUid: string): Promise<void> {
    ...
    if (!profile) {
      logger.warn('Cannot promote - profile not found', { firebaseUid });
    ...
    logger.info('User promoted to admin', { firebaseUid, newRoles: updatedRoles });
  }
  ...
  async revokeAdminAccess(firebaseUid: string): Promise<void> {
    ...
    const profile = await this.userProfileService.findByFirebaseUid(firebaseUid);
    if (!profile) {
      logger.warn('Cannot revoke - profile not found', { firebaseUid });
      throw new NotFoundException('User not found');
    }
    ...
    logger.info('Admin access revoked', { firebaseUid, newRoles: updatedRoles });
  }
  ...
  async getRoles(firebaseUid: string): Promise<UserRole[]> {
    ...
    if (!profile) {
      logger.warn('User roles lookup failed - user not found', { firebaseUid });
      throw new NotFoundException('User not found');
    }
